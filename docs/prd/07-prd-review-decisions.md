...
- 标准目录文档前半部分说，照片应以 `20260101-HongKong_Victoria_Peak` 作为命名依据。
- 后续默认模板示例又可能使用 `Victoria_Peak` 作为 `{title}`，生成 `20260101-Victoria_Peak-080001_DSC00000.ARW`。
- 如果没有 `.metadata.json` 或批量输入 `title`，工具到底应该解析出 `HongKong_Victoria_Peak`，还是进一步简化成 `Victoria_Peak`，目前没有规则。

需要决策：

- 默认解析是否只剥离日期，得到 `HongKong_Victoria_Peak`。
- 像 `Victoria_Peak` 这种短标题是否必须由 `.metadata.json` 或批量输入显式提供。

建议：

- 默认只剥离日期，不做地点或语义压缩。
- `Victoria_Peak` 这类短标题必须来自 `.metadata.json` 或批量输入，避免工具猜测地点语义。
